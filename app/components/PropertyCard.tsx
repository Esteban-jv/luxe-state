...
export default function PropertyCard({ property }: { property: Property }) {
  ...
  const tagClass = property.tag?.includes('RENT') 
    ? "bg-mosque/90 text-white" 
    : "bg-nordic-dark/90 text-white";
  ...
  return (
    <article className="bg-white dark:bg-white/5 rounded-xl overflow-hidden shadow-card hover:shadow-soft transition-all duration-300 group cursor-pointer h-full flex flex-col">
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {property.tag && (
          <div className={`absolute bottom-3 left-3 text-xs font-bold px-2 py-1 rounded ${tagClass}`}>
            {property.tag}
          </div>
        )}
      </div>
